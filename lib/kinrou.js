...
module.exports = {
    dakoku: async function(app, puppeteer, mainWindow, conf) {
        ...
        const dakokuListUrl = buildDakokuListUrl();
    ...
    buildDakokuListUrl: function() {
        const d = new Date();
        const year = dateformat(d, 'yyyy');
        const month = dateformat(d, 'mm');
        const kijunDate = dateformat(d, 'yyyymmdd');
        return `${kinrouUrl}/dakokuList/index?syainCode=${conf.userId}&year=${year}&month=${month}&kijunDate=${kijunDate}`;
    }
};
